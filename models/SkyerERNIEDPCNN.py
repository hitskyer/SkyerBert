...
class Config(object):
        ...
        def __init__(self,dataset):
            # 模型名称
            self.model_name = "SkyerERNIEDPCNN"
            # 训练集
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.fc = nn.Linear(config.num_filters, config.num_filters)
    def forward(self, x):
        context = x[0] # shape[128,32]
        mask = x[2] # shape[128, 32]
        encoder_out, text_cls = self.bert(context, attention_mask=mask, output_all_encoded_layers=False)
        # encoder_out.shape[128,32,768], text_cls.shape[128,768]
        out = encoder_out.unsqueeze(1) # shape[128,1,32,768]
        out = self.conv_region(out) # shape[128, 250, 30, 1]

        out = self.padd1(out) # shape[128, 250, 32, 1]
        out = self.relu(out)
        out = self.conv(out) # shape[128, 250, 30, 1]
        out = self.padd1(out) # shape[128, 250, 32, 1]
        out = self.relu(out)
        out = self.conv(out) # shape[128, 250, 30, 1]
        while out.size()[2] > 2: # 直到shape[128, 250, 1, 1]
            out = self._block(out)
        out = out.squeeze()
        out = self.fc(out)
        return out
    def _block(self, x):
        x = self.padd2(x) # [128, 250, 31, 1]
        px = self.max_pool(x) # [128, 250, 15, 1]
