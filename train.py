...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    optimizer = BertAdam(params=optimizer_grouped_parameters,
                         ...
                         warmup=0.05,
                         t_total=len(train_iter)*config.num_epoches)
    total_batch = 0 # 记录运行多少batch
    dev_best_loss = float('inf') # 记录开发集最好的loss
    last_improve = 0 # 记录上次校验集loss下降的batch数
    flag = False # 记录是否很久没有提升效果，如果是，则停止训练
    model.train()
    for epoch in range(config.num_epoches):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if total_batch%100 == 0:
                ...
                print(msg.format(total_batch, loss.item(), train_acc, dev_loss, dev_loss, dev_acc, time_diff, improve))
                model.train()
            total_batch += 1
            if total_batch-last_improve > config.require_improvement:
                # 在验证集合上loss超过1000batch没有下降，提前结束训练
                print('在验证集合上loss超过1000batch没有下降，提前结束训练')
                flag = True
                break
        if flag:
            break
